...
- **Committed, generated locally** — the icon set (`npm run icons`) and the share cards
  ...
  Who appears on the site-wide card is `OG_SITE_QUARTET` in `src/lib/site.js`, which the
  `og:image:alt` text is also derived from — so **editing it means rerunning `npm run og`**. The
  build fails if you forget (`assets/og-quartet.json` records what was drawn). Try variants in
  `npm run og-tool` first.
